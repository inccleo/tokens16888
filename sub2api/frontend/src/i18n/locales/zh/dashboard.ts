export default {
  dashboard: {
    ...
    rechargeNow: '去充值',
    createKey: '创建密钥',
    createFirstKeyTitle: '创建第一个 API Key',
    createFirstKeyDesc: '生成密钥后即可开始调用模型',
    firstCallTitle: '发起第一次调用',
    firstCallDesc: '复制下方示例，替换成你的密钥后即可运行',
    noBalanceTitle: '暂无可用余额',
    noBalanceDesc: '充值后即可开始使用 API',
    lowBalanceWarning: '余额偏低，建议尽快充值',
    serviceStatus: '服务状态',
    serviceOperational: '平台 API 可连接',
    serviceChecking: '正在检查平台 API',
    serviceUnavailable: '平台 API 暂不可连接',
    serviceStatusHint: '此检查仅确认平台 API 可达，不代表所有上游模型均正常。',
    retryServiceCheck: '重新检查',
    viewServiceStatus: '查看渠道状态',
    announcementsTitle: '系统公告',
    noAnnouncements: '暂无公告',
    viewAll: '查看全部',
    detailedMetrics: '更多指标',
    usageInsights: '用量洞察',
    browseModels: '浏览模型',
    browseModelsDesc: '查看可用模型与价格',
    quickStart: '快速开始',
    quickStartDesc: '三步开始你的第一次调用',
    baseUrl: 'API 地址',
    copyExample: '复制示例',
    stepCreateKey: '创建并复制 API Key',
    stepCopyExample: '复制下方调用示例',
    stepSendRequest: '替换密钥并发起请求',
    loadError: '数据加载失败',
    retry: '重试',
    apiKeys: 'API 密钥',
    todayRequests: '今日请求',
  ...
  docsHub: {
    title: '开发者文档',
    subtitle: '从创建 API Key 到发起第一条模型请求。',
    quickStart: '快速开始',
    stepOneTitle: '创建 API Key',
    stepOneDescription: '为项目创建独立密钥，并在弹出的使用说明中选择客户端配置。',
    stepTwoTitle: '选择可用模型',
    stepTwoDescription: '查看当前分组可用模型、价格和适用的调用协议。',
    stepThreeTitle: '发起 API 请求',
    stepThreeDescription: '替换模型名和密钥后运行示例。请先确认所选模型支持此兼容接口。',
    endpoint: 'API Base URL',
    openAIExample: 'OpenAI 兼容接口示例',
    copyExample: '复制示例',
    copied: '请求示例已复制',
    apiKeyPlaceholder: 'YOUR_API_KEY',
    modelPlaceholder: 'MODEL_ID',
    relatedGuides: '相关指南',
    batchImageGuide: '批量图片生成指南',
    configuredDocs: '站点文档',
    docsNotConfigured: '管理员尚未配置外部文档链接。',
  },
  ...
  modelPlaza: {
    ...
    noFavorites: '还没有收藏的模型，点击模型左侧的星标即可收藏',
    favorite: '收藏',
    unfavorite: '取消收藏',
